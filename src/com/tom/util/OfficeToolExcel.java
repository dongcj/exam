...
public class OfficeToolExcel {
	public static void makeExcel(String filepath, String title,
			List<Map<Object, Object>> list, int[] cell_length) throws Exception {
		...
		for (Map map : list) {
			int col = 0;
			for (Iterator localIterator2 = map.keySet().iterator(); localIterator2
					.hasNext();) {
				Object obj = localIterator2.next();
				if (row == 2) {
					Label label = new Label(col, row, String.valueOf(obj),
							getTitle());
					ws.addCell(label);
				} else {
					ws.addCell(new Label(col, row,
							String.valueOf(map.get(obj)), getNormolCell()));
				}
